...
function onCreate()
	if songName == 'Roleplay' then
		makeLuaSprite('bfUsername', 'roleplay/bfrobloxcensorshiptag', getProperty('boyfriend.x')+165, getProperty('boyfriend.y')-65)
...
function goodNoteHit(id, dir, nt, sus)
	if songName == 'Roleplay' then
		if dir == 0 then
			doTweenX('leftXBF', 'bfUsername', getProperty('boyfriend.x')+140, 0.001)
			doTweenY('leftYBF', 'bfUsername', getProperty('boyfriend.y')-55, 0.001)
		elseif dir == 1 then
			doTweenX('downXBF', 'bfUsername', getProperty('boyfriend.x')+145, 0.001)
			doTweenY('downYBF', 'bfUsername', getProperty('boyfriend.y')-15, 0.001)
		elseif dir == 2 then 
			doTweenX('upXBF', 'bfUsername', getProperty('boyfriend.x')+165, 0.001)
			doTweenY('upYBF', 'bfUsername', getProperty('boyfriend.y')-75, 0.001)
		elseif dir == 3 then 
			doTweenX('rightXBF', 'bfUsername', getProperty('boyfriend.x')+175, 0.001)
...
function onUpdate()
	if songName == 'Roleplay' then
		if getProperty("boyfriend.animation.curAnim.name") == "idle" and antimemoryleaklolbf == true then
			doTweenX('returnXBF', 'bfUsername', getProperty('boyfriend.x')+165, 0.001)
