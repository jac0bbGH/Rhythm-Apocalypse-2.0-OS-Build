...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		if (titleJSON.backgroundSprite != null && titleJSON.backgroundSprite.length > 0 && titleJSON.backgroundSprite != "none"){
			...
			FlxTween.tween(bg, {angle: 360}, 30, {type: PINGPONG});
		}else{
		...
		titlestatebg.velocity.set(200, 0);
		FlxTween.tween(titlestatebg, {angle: -360}, 60, {type: PINGPONG});
		titlestatebg.updateHitbox();
		titlestatebg.alpha = 0.2;
		titlestatebg.screenCenter(X);
		add(titlestatebg);
		FlxTween.tween(titlestatebg, {angle: 360}, 30, {type: PINGPONG});
		...
		blackScreen = new FlxSprite().loadGraphic(Paths.image('lololo'));
		...
		blackScreen2.screenCenter(X);
		FlxTween.tween(blackScreen2, {angle: -360}, 60, {type: PINGPONG});
		credGroup.add(blackScreen2);
		blackScreen2.shader = swagShader.shader;
	...
	override function update(elapsed:Float)
	{
		...
		if (initialized && !transitioning && skippedIntro)
		{
			...
			if(pressedEnter)
			{
				...
				transitioning = true;
				FlxTween.tween(logoBl, {"scale.x": 0, "scale.y": 0}, Conductor.crochet / 500, {ease: FlxEase.backIn});
				FlxTween.tween(bg, {"scale.x": 2, "scale.y": 2}, Conductor.crochet / 150, {ease: FlxEase.backIn});
				FlxTween.tween(blackScreen, {"scale.x": 2, "scale.y": 2}, Conductor.crochet / 150, {ease: FlxEase.backIn});
				FlxTween.tween(logoBl, {angle: -10, y: 500}, Conductor.crochet / 300, {ease: FlxEase.backIn});
